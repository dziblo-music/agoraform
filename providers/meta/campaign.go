...
const (
	campaignFields = "id,account_id,name,objective,status,configured_status,effective_status,special_ad_categories,buying_type,daily_budget,lifetime_budget,bid_strategy"
...
func (p *Provider) createCampaign(ctx context.Context, res resource.Resource) (resource.RemoteResource, error) {
	...
	form, err := campaignForm(normalized, true)
	if err != nil {
...
func (p *Provider) updateCampaign(ctx context.Context, desired resource.Resource, actual resource.RemoteResource) (resource.RemoteResource, error) {
	...
	form := url.Values{}
	if want.Name != got.Name { form.Set("name", want.Name) }
	if want.Status != got.Status { form.Set("status", want.Status) }
	...
	if want.HasDailyBudget && want.DailyBudget != got.DailyBudget { form.Set("daily_budget", strconv.FormatInt(want.DailyBudget, 10)) }
	if want.HasLifetimeBudget && want.LifetimeBudget != got.LifetimeBudget { form.Set("lifetime_budget", strconv.FormatInt(want.LifetimeBudget, 10)) }
	if want.HasBidStrategy && (!got.HasBidStrategy || want.BidStrategy != got.BidStrategy) { form.Set("bid_strategy", want.BidStrategy) }
	if len(form) == 0 {
		return p.readCampaignByID(ctx, desired.Address, actual.Identity.ID)
	}
	c, err := p.Client()
	if err != nil { return resource.RemoteResource{}, err }
...
func (p *Provider) importCampaign(ctx context.Context, addr resource.Address, rawID string) (resource.RemoteResource, error) {
	...
	if err != nil { return resource.RemoteResource{}, err }
	if err := p.requireConfig(); err != nil { return resource.RemoteResource{}, fmt.Errorf("meta: import %s: %w", addr, err) }
	live, err := p.readCampaignByID(ctx, addr, id)
	if err != nil {
		if errors.Is(err, provider.ErrNotFound) { return resource.RemoteResource{}, fmt.Errorf("meta: import %s: remote campaign %q was not found: %w", addr, id, err) }
...
func (p *Provider) normalizeCampaignComparable(desired resource.Resource, live *resource.RemoteResource) (resource.Attributes, resource.Attributes, error) {
	want, err := normalizeCampaign(desired)
	if err != nil { return nil, nil, fmt.Errorf("resource %s: %w", desired.Address, err) }
	wantAttrs := campaignAttributes(want)
	if live == nil { return wantAttrs, nil, nil }
	if id, bound, err := boundIdentity(desired); err != nil { return nil, nil, err
	...
	if err != nil { return nil, nil, fmt.Errorf("resource %s: live campaign is invalid: %w", desired.Address, err) }
	if err := validateCampaignTransition(desired.Address, want, got); err != nil { return nil, nil, err }
...
func (p *Provider) readCampaignByID(ctx context.Context, addr resource.Address, id string) (resource.RemoteResource, error) {
	...
	if err := c.Get(ctx, id, url.Values{"fields": {campaignFields}}, &item); err != nil {
		if client.IsNotFound(err) { return resource.RemoteResource{}, provider.ErrNotFound }
		return resource.RemoteResource{}, err
	}
	status := strings.ToUpper(strings.TrimSpace(item.ConfiguredStatus))
	if status == "" { status = strings.ToUpper(strings.TrimSpace(item.Status)) }
	if status == campaignStatusDeleted || status == campaignStatusArchived { return resource.RemoteResource{}, provider.ErrNotFound }
...
func (p *Provider) remoteCampaign(addr resource.Address, item campaign) (resource.RemoteResource, error) {
	...
	if err != nil { return resource.RemoteResource{}, fmt.Errorf("remote campaign id is invalid: %w", err) }
	if err := p.ensureCampaignAccount(item.AccountID); err != nil { return resource.RemoteResource{}, err }
	status := strings.ToUpper(strings.TrimSpace(item.ConfiguredStatus))
	if status == "" { status = strings.ToUpper(strings.TrimSpace(item.Status)) }
	attrs := resource.Attributes{
	...
	if attrs[AttrBuyingType] == "" { attrs[AttrBuyingType] = campaignBuyingAuction }
	if budget, set, err := remoteBudget(item.DailyBudget); err != nil { return resource.RemoteResource{}, fmt.Errorf("remote campaign %s has invalid daily_budget: %w", id, err)
	} else if set { attrs[AttrDailyBudget] = budget }
	if budget, set, err := remoteBudget(item.LifetimeBudget); err != nil { return resource.RemoteResource{}, fmt.Errorf("remote campaign %s has invalid lifetime_budget: %w", id, err)
	} else if set { attrs[AttrLifetimeBudget] = budget }
	if bid := strings.ToUpper(strings.TrimSpace(item.BidStrategy)); bid != "" { attrs[AttrBidStrategy] = bid }
	res := resource.Resource{Address: addr, Attributes: attrs}
	if _, err := normalizeCampaign(res); err != nil { return resource.RemoteResource{}, fmt.Errorf("remote campaign %s cannot be represented: %w", id, err) }
	computed := resource.Attributes{OutputCampaignID: id}
	if effective := strings.ToUpper(strings.TrimSpace(item.EffectiveStatus)); effective != "" { computed["effectiveStatus"] = effective }
...
func normalizeCampaign(res resource.Resource) (normalizedCampaign, error) {
	...
	if err != nil { return normalizedCampaign{}, err }
	...
	if err != nil { return normalizedCampaign{}, fmt.Errorf("%w; RESERVED campaigns require a materially different schema and are out of scope", err) }
	...
	if err != nil { return normalizedCampaign{}, err }
	if hasDaily && hasLifetime { return normalizedCampaign{}, fmt.Errorf("resource %s: attributes %q and %q are mutually exclusive", res.Address, AttrDailyBudget, AttrLifetimeBudget) }
	bid, err := campaignEnum(res, AttrBidStrategy, campaignBidStrategies, false, "")
	if err != nil { return normalizedCampaign{}, err }
	_, hasBid := res.Attributes[AttrBidStrategy]
	if hasBid && !hasDaily && !hasLifetime { return normalizedCampaign{}, fmt.Errorf("resource %s: attribute %q requires %q or %q at campaign level", res.Address, AttrBidStrategy, AttrDailyBudget, AttrLifetimeBudget) }
	return normalizedCampaign{Name: name, Objective: objective, Status: status, SpecialAdCategories: categories, BuyingType: buyingType,
		...
		BidStrategy: bid, HasBidStrategy: hasBid}, nil
}
...
func campaignEnum(res resource.Resource, key string, allowed map[string]struct{}, required bool, defaultValue string) (string, error) {
	...
	if !ok {
		if required { return "", fmt.Errorf("resource %s: missing required attribute %q", res.Address, key) }
		return defaultValue, nil
	}
	s, err := coerceString(v)
	if err != nil || strings.TrimSpace(s) == "" { return "", fmt.Errorf("resource %s: attribute %q must be a non-empty string", res.Address, key) }
	s = strings.ToUpper(strings.TrimSpace(s))
	if _, ok := allowed[s]; !ok { return "", fmt.Errorf("resource %s: attribute %q must be one of %s", res.Address, key, joinSorted(keys(allowed))) }
	return s, nil
}

func requiredCategories(res resource.Resource) ([]string, error) {
	v, ok := res.Attributes[AttrSpecialAdCategories]
	if !ok { return nil, fmt.Errorf("resource %s: missing required attribute %q; use an empty list when no special-ad category applies", res.Address, AttrSpecialAdCategories) }
	rv := reflect.ValueOf(v)
	if !rv.IsValid() || (rv.Kind() != reflect.Slice && rv.Kind() != reflect.Array) { return nil, fmt.Errorf("resource %s: attribute %q must be a list", res.Address, AttrSpecialAdCategories) }
	set := map[string]struct{}{}
	for i := 0; i < rv.Len(); i++ {
		s, err := coerceString(rv.Index(i).Interface())
		if err != nil || strings.TrimSpace(s) == "" { return nil, fmt.Errorf("resource %s: attribute %q[%d] must be a non-empty string", res.Address, AttrSpecialAdCategories, i) }
		s = strings.ToUpper(strings.TrimSpace(s))
		if s == "NONE" {
			if rv.Len() != 1 { return nil, fmt.Errorf("resource %s: NONE cannot be combined with another %q value", res.Address, AttrSpecialAdCategories) }
			return []string{}, nil
		}
		if _, ok := campaignSpecialAdCategories[s]; !ok { return nil, fmt.Errorf("resource %s: attribute %q[%d] must be one of %s", res.Address, AttrSpecialAdCategories, i, joinSorted(keys(campaignSpecialAdCategories))) }
		if _, duplicate := set[s]; duplicate { return nil, fmt.Errorf("resource %s: attribute %q contains duplicate value %q", res.Address, AttrSpecialAdCategories, s) }
		set[s] = struct{}{}
	}
	out := make([]string, 0, len(set)); for s := range set { out = append(out, s) }; sort.Strings(out); return out, nil
}

func optionalBudget(res resource.Resource, key string) (int64, bool, error) {
	v, ok := res.Attributes[key]
	if !ok { return 0, false, nil }
	n, err := coerceFloat(v)
...
func remoteBudget(v any) (int64, bool, error) {
	if v == nil || v == "" { return 0, false, nil }
	n, err := coerceFloat(v)
	if err != nil { return 0, true, err }
	if n == 0 { return 0, false, nil }
	if n < 0 || n != math.Trunc(n) || n > math.MaxInt64 { return 0, true, fmt.Errorf("must be a positive whole number") }
	return int64(n), true, nil
}
...
func campaignAttributes(c normalizedCampaign) resource.Attributes {
	...
	if c.HasDailyBudget { out[AttrDailyBudget] = c.DailyBudget }
	if c.HasLifetimeBudget { out[AttrLifetimeBudget] = c.LifetimeBudget }
	if c.HasBidStrategy { out[AttrBidStrategy] = c.BidStrategy }
	return out
}

func campaignForm(c normalizedCampaign, create bool) (url.Values, error) {
	form := url.Values{}
	form.Set("name", c.Name); form.Set("objective", c.Objective); form.Set("status", c.Status); form.Set("buying_type", c.BuyingType)
	raw, err := json.Marshal(c.SpecialAdCategories); if err != nil { return nil, err }; form.Set("special_ad_categories", string(raw))
	if c.HasDailyBudget { form.Set("daily_budget", strconv.FormatInt(c.DailyBudget, 10)) }
	if c.HasLifetimeBudget { form.Set("lifetime_budget", strconv.FormatInt(c.LifetimeBudget, 10)) }
	if c.HasBidStrategy { form.Set("bid_strategy", c.BidStrategy) }
	return form, nil
}

func validateCampaignTransition(addr resource.Address, want, got normalizedCampaign) error {
	if want.Objective != got.Objective { return fmt.Errorf("resource %s: objective is immutable/unsafe after create; create a new meta.campaign instead", addr) }
	if want.BuyingType != got.BuyingType { return fmt.Errorf("resource %s: buyingType is immutable after create; create a new meta.campaign instead", addr) }
	...
	if got.HasBidStrategy && !want.HasBidStrategy { return fmt.Errorf("resource %s: bidStrategy cannot be cleared deterministically through the Marketing API", addr) }
	return nil
}

func (p *Provider) ensureCampaignAccount(accountID string) error {
	c, err := p.Client(); if err != nil { return err }
	accountID = strings.TrimSpace(accountID); if accountID == "" { return nil }
	if strings.TrimPrefix(accountID, "act_") != strings.TrimPrefix(c.AdAccountID(), "act_") { return fmt.Errorf("campaign belongs to ad account %s, not the configured %s", accountID, c.AdAccountID()) }
	return nil
}

func normalizeCategories(in []string) []string {
	set := map[string]struct{}{}; for _, s := range in { s = strings.ToUpper(strings.TrimSpace(s)); if s != "" && s != "NONE" { set[s] = struct{}{} } }
	out := make([]string, 0, len(set)); for s := range set { out = append(out, s) }; sort.Strings(out); return out
}
...
func (p *Provider) destroyCampaign(ctx context.Context, res resource.Resource) (provider.DestroyResult, error) {
	id, bound, err := boundIdentity(res); if err != nil { return provider.DestroyResult{}, err }
	if !bound { return provider.DestroyResult{}, fmt.Errorf("meta: destroy %s: missing persisted identity", res.Address) }
	_, err = p.readCampaignByID(ctx, res.Address, id)
	if errors.Is(err, provider.ErrNotFound) { return provider.DestroyResult{Status: provider.DestroyStatusAlreadyAbsent}, nil }
	if err != nil { return provider.DestroyResult{}, fmt.Errorf("meta: destroy %s: %w", res.Address, err) }
	c, err := p.Client(); if err != nil { return provider.DestroyResult{}, err }
	var result map[string]any
	if err := c.Delete(ctx, id, nil, &result); err != nil {
		if client.IsNotFound(err) { return provider.DestroyResult{Status: provider.DestroyStatusAlreadyAbsent}, nil }
		return provider.DestroyResult{}, fmt.Errorf("meta: destroy %s: %w", res.Address, err)
	}
	if success, ok := result["success"].(bool); ok && !success { return provider.DestroyResult{}, fmt.Errorf("meta: destroy %s: API did not report success", res.Address) }
	_, err = p.readCampaignByID(ctx, res.Address, id)
	if errors.Is(err, provider.ErrNotFound) { return provider.DestroyResult{Status: provider.DestroyStatusRemoved}, nil }
	if err != nil { return provider.DestroyResult{}, fmt.Errorf("meta: destroy %s: DELETE succeeded but confirming terminal state failed: %w", res.Address, err) }
