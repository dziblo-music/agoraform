...
const googleAdsCampaignNegativeKeywordManifest = `apiVersion: agoraform.io/v1alpha1
resources:
  - address: googleads.campaign_budget.brand
    attributes:
      name: Brand daily budget
...
func TestValidateGoogleAdsCampaignNegativeKeywordMissingCredentials(t *testing.T) {
	t.Parallel()

	path := writeManifest(t, "agoraform.yaml", googleAdsCampaignNegativeKeywordManifest)
	streams, _, stderr := testStreams()
	code := cli.ExecuteWith(streams, []string{"validate", "-f", path})
	if code != cli.ExitError {
		t.Fatalf("exit code = %d, want %d; stderr=%q", code, cli.ExitError, stderr.String())
	...
	errOut := stderr.String()
	if strings.Contains(errOut, "unknown resource type") {
		t.Fatalf("stderr = %q, googleads.campaign_negative_keyword should be registered", errOut)
	}
	if !strings.Contains(errOut, googleads.EnvDeveloperToken) && !strings.Contains(errOut, "required") {
		t.Fatalf("stderr = %q, want missing credential error", errOut)
	}
...
func TestValidateGoogleAdsCampaignNegativeKeywordWithProvider(t *testing.T) {
	t.Parallel()
