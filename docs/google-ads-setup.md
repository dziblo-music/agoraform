...
Agoraform uses the Google Ads API through OAuth 2.0 **single-user
...
does not implement those authentication modes; supply a client ID,
...
can affect optimization and spend. The v0.5.0 release checklist requires
...
For the v0.5.0 Matomo + Google Ads lifecycle example, including destroy,
see [`examples/matomo-googleads/`](../examples/matomo-googleads/README.md).
