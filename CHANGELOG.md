...
- `googleads.keyword` for Search ad-group keyword criteria, including negative keywords (read, create, update, import). Keywords reference ad groups with `$ref: googleads.ad_group.*` and declare `text`, `matchType` (`EXACT`, `PHRASE`, `BROAD`), and optional `negative`. Text is normalized without changing user intent. Text, match type, negative, and ad group are immutable; plan reports that instead of hiding replacement. Optional max CPC bid overrides are declared in account-currency units. New keywords default to `PAUSED`. Equivalent remote state produces no plan diff. Provider-native IDs and resource names remain computed.
